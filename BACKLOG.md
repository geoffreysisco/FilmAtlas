# FilmAtlas Development Punch List

==============================
MUST FIX (true defects / broken behavior)
==============================

1. Search pill clear behavior
 When in search mode and text exists in the search pill, there should be an **X** to clear the text after focus is restored.
 Anytime text exists there must also be a way to delete it.
 Repro: Always
 Scope: Medium
...
8. Back button leaves invalid UI state
 In search mode with results present, pressing back restores the original tab name but leaves the RecyclerView showing a network error message while on the browse tabs (Discover, Popular, and New).
 Repro: Always
 Scope: Medium

9. Add Favorites heart and Share icon to landscape movie dialog
 There is currently no way to favorite or share while in landscape mode. The buttons need to be added and wired correctly.
 Repro: Always
 Scope: Medium
...
3. Movie dialog animation polish
 Add a subtle bounce/settle animation on dialog entrance.
 Repro: Always
 Scope: Quick

4. Improve filter button visual depth
 Consider adding stroke or elevation to reduce flat appearance.
 Repro: Always
 Scope: Quick
...
Icon color is currently chosen dynamically (black or white) based on poster brightness.
...
16. Add scrim behind pill buttons on movie dialog
The pill buttons (where to watch, trailer, TMDB rating) need scrim and possibly stroke
Repro: Always
Scope: Medium
...
3. Move history pseudo-movie creation into a helper or mapper.
